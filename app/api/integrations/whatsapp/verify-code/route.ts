...
export async function POST(request: Request) {
  try {
    ...
    const number = normalizeWhatsappNumber((normalizeUtf8Text(body.number || "") as string) || "");
    const code = ((normalizeUtf8Text(body.code || "") as string) || "").trim().toUpperCase();

    if (!number || !code || code.length !== 4) {
      return jsonUtf8({ success: false, error: "Codigo invalido" }, { status: 400 });
    }
    ...
    if (!row?.whatsapp_verification_code || !row.whatsapp_verification_expires_at) {
      return jsonUtf8({ success: false, error: "No hay verificacion pendiente" }, { status: 400 });
    }

    if (normalizeWhatsappNumber(row.whatsapp_number || "") !== number) {
      return jsonUtf8(
        { success: false, error: "El numero no coincide con la verificacion pendiente" },
        { status: 400 }
      );
    }
    ...
    if (expired) {
      return jsonUtf8({ success: false, error: "El codigo expiro. Solicita uno nuevo." }, { status: 400 });
    }
    ...
    if (row.whatsapp_verification_code.toUpperCase() !== code) {
      ...
      await supabaseAdmin
        ...
        .eq("user_id", user.id);
      return jsonUtf8({ success: false, error: "Codigo incorrecto" }, { status: 400 });
    }
