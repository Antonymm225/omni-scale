  ...
  const message = useMemo(() => {
    if (status === "success") {
      return {
        type: "success",
        text: isEn
          ? "Connection completed. Your Facebook assets were synced."
          : "Conexion completada. Se sincronizaron tus assets de Facebook.",
      };
    }
    ...
    if (status === "error") {
      return {
        ...
        text:
          ...
          (isEn ? "Could not complete Facebook connection." : "No se pudo completar la conexion con Facebook."),
      };
    }

    return null;
  }, [searchParams, status, isEn]);
      ...
      <div className="mx-auto w-full max-w-3xl">
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-8 shadow-sm sm:p-10">
          <h1 className="text-center text-3xl font-bold tracking-tight text-[#1D293D]">
            {isEn ? "Connect your Meta assets" : "Conecta tus assets de Meta"}
          </h1>
          <p className="mt-3 text-center text-sm text-slate-600 sm:text-base">
            {isEn
              ? "Connect Facebook to import BMs, ad accounts, pixels, fanpages, Instagram accounts, and adsets."
              : "Conecta Facebook para importar BM, cuentas publicitarias, pixels, fanpages, cuentas de Instagram y adsets."}
          </p>

          {message && (
            <div className={`mt-6 rounded-xl border px-4 py-3 text-sm ${message.type === "success" ? "border-emerald-200 bg-emerald-50 text-emerald-700" : "border-red-200 bg-red-50 text-red-700"}`}>
              {message.text}
            </div>
          )}

          <a href="/api/facebook/oauth/start" className="mt-8 inline-flex w-full items-center justify-center rounded-xl bg-[#1D293D] px-4 py-3 text-sm font-semibold text-white transition hover:opacity-90">
            {isEn ? "Connect with Facebook" : "Conectar con Facebook"}
          </a>
          ...
          <p className="mt-4 text-center text-xs text-slate-500">
            {isEn
              ? "We will request permissions to read and sync the assets you manage."
