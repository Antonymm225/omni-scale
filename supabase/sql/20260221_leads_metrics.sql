-- Leads-only metrics cache (summary per user)
...
begin
  if not exists (
    ...
    where schemaname = 'public'
      and tablename = 'facebook_leads_metrics'
      and policyname = 'facebook_leads_metrics_owner_select'
  ) then
    create policy facebook_leads_metrics_owner_select
      on public.facebook_leads_metrics for select
      using (auth.uid() = user_id);
  end if;
end $$;

-- Leads-only metrics cache by ad account
...
begin
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public'
      and tablename = 'facebook_leads_ad_account_metrics'
      and policyname = 'facebook_leads_ad_account_metrics_owner_select'
  ) then
    create policy facebook_leads_ad_account_metrics_owner_select
      on public.facebook_leads_ad_account_metrics for select
      using (auth.uid() = user_id);
  end if;
end $$;

-- Leads intraday timeseries (every 10min snapshot)
...
begin
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public'
      and tablename = 'facebook_leads_timeseries'
      and policyname = 'facebook_leads_timeseries_owner_select'
  ) then
    create policy facebook_leads_timeseries_owner_select
      on public.facebook_leads_timeseries for select
      using (auth.uid() = user_id);
  end if;
end $$;
