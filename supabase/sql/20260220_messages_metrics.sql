-- Messaging-only metrics cache (summary per user)
...
begin
  if not exists (
    ...
    where schemaname = 'public'
      and tablename = 'facebook_messages_metrics'
      and policyname = 'facebook_messages_metrics_owner_select'
  ) then
    create policy facebook_messages_metrics_owner_select
      on public.facebook_messages_metrics for select
      using (auth.uid() = user_id);
  end if;
end $$;

-- Messaging-only metrics cache by ad account
...
begin
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public'
      and tablename = 'facebook_messages_ad_account_metrics'
      and policyname = 'facebook_messages_ad_account_metrics_owner_select'
  ) then
    create policy facebook_messages_ad_account_metrics_owner_select
      on public.facebook_messages_ad_account_metrics for select
      using (auth.uid() = user_id);
  end if;
end $$;

-- Messaging intraday timeseries (every 10min snapshot)
...
begin
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public'
      and tablename = 'facebook_messages_timeseries'
      and policyname = 'facebook_messages_timeseries_owner_select'
  ) then
    create policy facebook_messages_timeseries_owner_select
      on public.facebook_messages_timeseries for select
      using (auth.uid() = user_id);
  end if;
end $$;
