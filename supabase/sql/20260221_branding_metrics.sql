-- Branding-only metrics cache (summary per user)
...
begin
  if not exists (
    ...
    where schemaname = 'public'
      and tablename = 'facebook_branding_metrics'
      and policyname = 'facebook_branding_metrics_owner_select'
  ) then
    create policy facebook_branding_metrics_owner_select
      on public.facebook_branding_metrics for select
      using (auth.uid() = user_id);
  end if;
end $$;

-- Branding-only metrics cache by ad account
...
begin
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public'
      and tablename = 'facebook_branding_ad_account_metrics'
      and policyname = 'facebook_branding_ad_account_metrics_owner_select'
  ) then
    create policy facebook_branding_ad_account_metrics_owner_select
      on public.facebook_branding_ad_account_metrics for select
      using (auth.uid() = user_id);
  end if;
end $$;

-- Branding intraday timeseries (every 10min snapshot)
...
begin
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public'
      and tablename = 'facebook_branding_timeseries'
      and policyname = 'facebook_branding_timeseries_owner_select'
  ) then
    create policy facebook_branding_timeseries_owner_select
      on public.facebook_branding_timeseries for select
      using (auth.uid() = user_id);
  end if;
end $$;
